# TODO Update main to use other modules
import json
import os
import sys
import hashlib
...
class PeerMessage:
    def __init__(self, message_id: bytes, payload: bytes):
        ...
        self.message_length_prefix = (
            len(message_id + payload)).to_bytes(4, byteorder="big")
...
class Tracker:
    ...
    def get_peers(self, info_hash: bytes, peer_id: str, port: int = 6881, uploaded: int = 0, downloaded: int = 0, left: int = 0, compact: int = 1) -> requests.Response:
        response = requests.get(self.announce_url,
                                params={
                                    'info_hash': info_hash,
                                    'peer_id': peer_id,
                                    'port': port,
...
class Peer:
    def __init__(self):
        ...
        self.protocol_length = len(self.protocol).to_bytes(
            1, byteorder="big")  # 1 byte
    ...
    def handshake(self, info_hash: bytes, peer_id: bytes) -> bytes:
        ...
        handshake_msg = self.protocol_length + \
            self.protocol + self.reserved + info_hash + peer_id
    ...
    def bitfield_listen(self) -> list[int]:
        ...
        # print(payload_str)
        indexes_of_pieces = [
            i for i, bit in enumerate(payload_str) if bit == "1"]
...
def handle_download_piece(download_directory, torrent_file_name, piece):
    ...
    piece_hash = meta_info.get_piece_hashes()[piece]
    calculated_piece_hash = hashlib.sha1(block).digest().hex()
    assert piece_hash == calculated_piece_hash, f"Piece hash mismatch!"
